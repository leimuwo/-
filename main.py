...
class Operate(object):
    ...
    def show_pic(self):
        ...
        # print(img)
        # 获取self.canva_show对象的宽度和高度，并转换为整数类型，赋值给canvawidth和canvaheight变量。
        canvawidth = int(self.canva_show.winfo_reqwidth())
        canvaheight = int(self.canva_show.winfo_reqheight())    

        # 调整图片的尺寸，inter_area是一种插值方法，使用像素区域的关系进行重采样
    ...
    def save_pic(self):
        ...
        if self.combobox.get() == "一寸":
            ...
            self.save_width = 295
            self.save_img = self.save_img.resize((self.save_width, self.save_height), resample=Image.Resampling.BILINEAR)
